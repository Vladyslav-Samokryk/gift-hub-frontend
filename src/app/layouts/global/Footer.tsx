...
import {
  ApplePayLogo, EmailLogo, GooglePayLogo, LocationPointLogo, MainLogo, MasterCardLogo, TelegramLogo, TikTokLogo, TweeterLogo, VisaLogo, WhatsAppLogo, useTypedTranslation,
} from "@shared";
import { CONTACT_INFO } from "@config";
import { FacebookLogo } from "@src/shared/assets/svg/FacebookLogo";
  ...
  return (
    <footer className="grid grid-cols-3 divide-x divide-gray-400 bg-background-footer px-12 pt-8 font-rubik">
      ...
      <section className="flex flex-col items-center">
        ...
        <fieldset className="flex h-max gap-4">
          <legend className="pb-5">
            <strong className="text-primary-700">{t('follow_us')}:</strong>
          </legend>
        ...
        <fieldset className="flex flex-col items-start">
          ...
          <ul className="flex flex-col gap-3">
            {helpSection.map(({ key, path, translationKey }) => (
              <li key={key}>
                <Link to={path} className="font-light text-primary">{t(translationKey)}</Link>
              </li>
            ))}
          </ul>
        </fieldset>
      ...
      <section className="col-span-full flex items-center justify-between border-t px-11" style={{ borderLeft: "0px" }}>
        <small>&copy; {t('copyrightText')}</small>
